      ...
      <section className="lesson-modal" role="dialog" aria-modal="true" aria-labelledby="calendar-lesson-details-title">
        ...
        <form aria-busy={isMutating} onSubmit={(event) => { event.preventDefault(); void submitEdit(); }}>
          <div className="lesson-form-grid">
            ...
            <label><span>Время начала</span><input type="time" min="08:00" max="21:30" step="1800" value={draft.startTime} onChange={(event) => updateStartTime(event.target.value)} required disabled={isMutating} /></label>
            <label><span>Время окончания</span><input type="time" min="08:30" max="22:00" step="1800" value={draft.endTime} onChange={(event) => update("endTime", event.target.value)} required disabled={isMutating} /></label>
