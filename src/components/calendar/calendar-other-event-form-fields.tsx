...
export function CalendarOtherEventFormFields({ draft, disabled, onChange }: CalendarOtherEventFormFieldsProps) {
  return (
    <div className="lesson-form-grid">
      ...
      <label><span>Время начала</span><input type="time" step="60" value={draft.startTime} onChange={(event) => onChange("startTime", event.target.value)} required disabled={disabled} /></label>
      <label><span>Время окончания</span><input type="time" step="60" value={draft.endTime} onChange={(event) => onChange("endTime", event.target.value)} required disabled={disabled} /></label>
